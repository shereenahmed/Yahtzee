package yahtzee.game.controller;
...
import yahtzee.game.model.Die;
import yahtzee.game.model.ScoreSheet;
import yahtzee.game.view.myPanel;
...
public class Yahtzee {
	...
	Die dice[] = new Die[5]; 
	ScoreSheet scoreCard[] = new ScoreSheet[6]; 
	...
	public void setupSound() {
		...
		try {
			AudioInputStream yahtzeeSound = AudioSystem
					.getAudioInputStream(yahtzeeFile);
		...
		public void keyPressed(KeyEvent e) {
			if (rolling) {
				...
				if (e.getKeyCode() == KeyEvent.VK_A
						&& diceButtons[0].isEnabled())
					toggleDie(0);
				if (e.getKeyCode() == KeyEvent.VK_S
						&& diceButtons[1].isEnabled())
					toggleDie(1);
				if (e.getKeyCode() == KeyEvent.VK_D
						&& diceButtons[2].isEnabled())
					toggleDie(2);
				if (e.getKeyCode() == KeyEvent.VK_F
						&& diceButtons[3].isEnabled())
					toggleDie(3);
				if (e.getKeyCode() == KeyEvent.VK_G
						&& diceButtons[4].isEnabled())
					toggleDie(4);
	...
	private class AppListener implements PreferencesHandler {
		public void handlePreferences(AppEvent.PreferencesEvent e) {
			...
			prefPanel
					.add(new JLabel(
							"<html><p>  Adjust the speed of the rolling animation to increase or lower the difficulty.<br>  500 is very easy, 250 is easy, 60 is normal.</p></html>",
